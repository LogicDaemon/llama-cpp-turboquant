...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        ggml_metal_op_mul_mat_use_mm(op, props_dev->has_simdgroup_mm) ||
        (is_tq_weight && props_dev->has_simdgroup_mm && ne00 >= 64 && ne11 >= 1)) {
...
int ggml_metal_op_mul_mat_id(ggml_metal_op_t ctx, int idx) {
    ...
    if (ggml_metal_op_mul_mat_id_use_mm(op, props_dev->has_simdgroup_mm) ||
        (is_tq_weight && props_dev->has_simdgroup_mm && ne00 >= 64 && ne21 >= ne21_mm_id_min)) {
...
static bool ggml_metal_op_flash_attn_ext_use_turbo_flash(const ggml_tensor * op) {
    ...
    const int64_t ne00 = op->src[0]->ne[0]; // head size
    const int64_t ne20 = op->src[2]->ne[0]; // value head size
    ...
    if (ggml_metal_op_flash_attn_ext_use_turbo_flash(op)) {
        ...
        // so turbo data starts at:
        {
            ...
            // ---- Pass 2: Merge partials + inverse WHT + write output ----
            {
                ...
                if (!res_p2.pipeline) {
                    // Fall through to normal FA if the second pass cannot be compiled.
                    goto turbo_flash_end;
                }
