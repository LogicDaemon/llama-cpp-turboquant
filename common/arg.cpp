...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--cpu-tp"},
        "include the CPU as a tensor-parallel device (requires --split-mode tensor).\n"
        "every weight matrix is split by rows between the GPU(s) and the CPU so both compute concurrently.\n"
        "the GPU share is sized automatically to saturate VRAM, the remainder goes to the CPU",
        [](common_params & params) {
            params.cpu_tp = true;
        }
    ).set_env("LLAMA_ARG_CPU_TP"));
    add_opt(common_arg(
        {"--cpu-tp-margin"}, "N",
        string_format("MiB of VRAM to leave free per GPU when sizing the --cpu-tp split (default: %d)", params.cpu_tp_margin),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value");
            }
            params.cpu_tp_margin = value;
        }
    ).set_env("LLAMA_ARG_CPU_TP_MARGIN"));
